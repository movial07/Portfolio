...
// Deterministic pseudo-random shuffle helper using distinct seeds
const shuffleArray = (arr: DriftWallItem[], seed: number): DriftWallItem[] => {
  const result = [...arr];
  let m = result.length;
  let s = seed;
  while (m) {
...
const DriftWall = ({
  ...
  // Generate uniquely shuffled item sequences per column so vertical & horizontal card patterns are completely randomized
  const columnItems = useMemo<DriftWallItem[][]>(() => {
    if (!items.length) return [];
    return Array.from({ length: columns }, (_, cIndex) => {
      const seed = (cIndex + 1) * 7919 + 1337;
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className="drift-wall__inner group/tile">
        <img src={item.image} alt={item.title ?? ''} loading="lazy" decoding="async" draggable={false} />
        <span className="drift-wall__overlay" aria-hidden="true" />
        
        {/* On-Tile Explicit "Click to watch ↗" Label Badge on Active Hover */}
        <span className="drift-wall__action-badge">
          Click to watch ↗
        </span>
      </span>
    );
  ...
  return (
    ...
    >
      <div className="drift-wall__blur-top" aria-hidden="true" />
            ...
            <div className="drift-wall__col" key={`col-${c}`}>
              ...
              >
                {Array.from({ length: meta.copies }).flatMap((_, copyIndex) => {
                  const copySeed = (c + 1) * 7919 + copyIndex * 104729;
                  const copyItems = shuffleArray(col, copySeed);
                  return copyItems.map((item, itemIndex) =>
      ...
      <div className="drift-wall__blur-bottom" aria-hidden="true" />
    </div>
  );
};
